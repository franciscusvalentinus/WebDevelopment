    ...
    <div>
        <div class="max-w-4xl mx-auto py-10 sm:px-6 lg:px-8">
            <div class="block mb-8">
                <a href="{{ route('design.index') }}" class="bg-gray-200 hover:bg-gray-300 text-black font-bold py-2 px-4 rounded">Back to list</a>
            </div>
                    ...
                    <div class="shadow overflow-hidden sm:rounded-md">
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <input type="text" name="judul_ciptaan" id="judul_ciptaan" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('judul_ciptaan', $design->judul_ciptaan) }}" autofocus/>
                            @error('judul_ciptaan')
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <input type="date" name="tanggal_permohonan" id="tanggal_permohonan" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('tanggal_permohonan', $design->tanggal_permohonan) }}" />
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <input type="text" name="nomor_permohonan" id="nomor_permohonan" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('nomor_permohonan', $design->nomor_permohonan) }}" />
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            <label for="pemohon" class="block font-medium text-sm text-gray-700">Pemohon</label>
                            <input type="text" name="pemohon" id="pemohon" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('pemohon', $design->pemohon) }}" />
                            @error('pemohon')
                            <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <input type="text" name="status" id="status" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('status', $design->status) }}" />
                            @error('status')
                            <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                                ...
                                @foreach($lecturers as $lecturer)
                                    @foreach ($departments as $department)
                                        @if ($lecturer->department_id == $department->id)
                                            <option value="{{ $lecturer->id }}" {{$design->pencipta_1 == $lecturer->id  ? 'selected' : ''}}>{{ $lecturer->name}} ({{ $department->department }}) ({{ $lecturer->nidn }})</option>
                                        @endif
                                    @endforeach
                                @endforeach
                            </select>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <select type="text" name="pencipta_2" id="pencipta_2" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                ...
                                @foreach($lecturers as $lecturer)
                                    @foreach ($departments as $department)
                                        @if ($lecturer->department_id == $department->id)
                                            <option value="{{ $lecturer->id }}" {{$design->pencipta_2 == $lecturer->id  ? 'selected' : ''}}>{{ $lecturer->name}} ({{ $department->department }}) ({{ $lecturer->nidn }})</option>
                                        @endif
                                    @endforeach
                                @endforeach
                            </select>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <select type="text" name="pencipta_3" id="pencipta_3" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                ...
                                @foreach($lecturers as $lecturer)
                                    @foreach ($departments as $department)
                                        @if ($lecturer->department_id == $department->id)
                                            <option value="{{ $lecturer->id }}" {{$design->pencipta_3 == $lecturer->id  ? 'selected' : ''}}>{{ $lecturer->name}} ({{ $department->department }}) ({{ $lecturer->nidn }})</option>
                                        @endif
                                    @endforeach
                                @endforeach
                            </select>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <select type="text" name="pencipta_4" id="pencipta_4" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                ...
                                @foreach($lecturers as $lecturer)
                                    @foreach ($departments as $department)
                                        @if ($lecturer->department_id == $department->id)
                                            <option value="{{ $lecturer->id }}" {{$design->pencipta_4 == $lecturer->id  ? 'selected' : ''}}>{{ $lecturer->name}} ({{ $department->department }}) ({{ $lecturer->nidn }})</option>
                                        @endif
                                    @endforeach
                                @endforeach
                            </select>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <select type="text" name="pencipta_5" id="pencipta_5" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                ...
                                @foreach($lecturers as $lecturer)
                                    @foreach ($departments as $department)
                                        @if ($lecturer->department_id == $department->id)
                                            <option value="{{ $lecturer->id }}" {{$design->pencipta_5 == $lecturer->id  ? 'selected' : ''}}>{{ $lecturer->name}} ({{ $department->department }}) ({{ $lecturer->nidn }})</option>
                                        @endif
                                    @endforeach
                                @endforeach
                            </select>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <select type="text" name="pencipta_6" id="pencipta_6" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                ...
                                @foreach($lecturers as $lecturer)
                                    @foreach ($departments as $department)
                                        @if ($lecturer->department_id == $department->id)
                                            <option value="{{ $lecturer->id }}" {{$design->pencipta_6 == $lecturer->id  ? 'selected' : ''}}>{{ $lecturer->name}} ({{ $department->department }}) ({{ $lecturer->nidn }})</option>
                                        @endif
                                    @endforeach
                                @endforeach
                            </select>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <select type="text" name="pencipta_7" id="pencipta_7" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                ...
                                @foreach($lecturers as $lecturer)
                                    @foreach ($departments as $department)
                                        @if ($lecturer->department_id == $department->id)
                                            <option value="{{ $lecturer->id }}" {{$design->pencipta_7 == $lecturer->id  ? 'selected' : ''}}>{{ $lecturer->name}} ({{ $department->department }}) ({{ $lecturer->nidn }})</option>
                                        @endif
                                    @endforeach
                                @endforeach
                            </select>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <select type="text" name="pencipta_8" id="pencipta_8" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                ...
                                @foreach($lecturers as $lecturer)
                                    @foreach ($departments as $department)
                                        @if ($lecturer->department_id == $department->id)
                                            <option value="{{ $lecturer->id }}" {{$design->pencipta_8 == $lecturer->id  ? 'selected' : ''}}>{{ $lecturer->name}} ({{ $department->department }}) ({{ $lecturer->nidn }})</option>
                                        @endif
                                    @endforeach
                                @endforeach
                            </select>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <select type="text" name="pencipta_9" id="pencipta_9" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                ...
                                @foreach($lecturers as $lecturer)
                                    @foreach ($departments as $department)
                                        @if ($lecturer->department_id == $department->id)
                                            <option value="{{ $lecturer->id }}" {{$design->pencipta_9 == $lecturer->id  ? 'selected' : ''}}>{{ $lecturer->name}} ({{ $department->department }}) ({{ $lecturer->nidn }})</option>
                                        @endif
                                    @endforeach
                                @endforeach
                            </select>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <select type="text" name="pencipta_10" id="pencipta_10" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                ...
                                @foreach($lecturers as $lecturer)
                                    @foreach ($departments as $department)
                                        @if ($lecturer->department_id == $department->id)
                                            <option value="{{ $lecturer->id }}" {{$design->pencipta_10 == $lecturer->id  ? 'selected' : ''}}>{{ $lecturer->name}} ({{ $department->department }}) ({{ $lecturer->nidn }})</option>
                                        @endif
                                    @endforeach
                                @endforeach
                            </select>
