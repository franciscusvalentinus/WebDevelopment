            ...
            <div class="px-6 py-4 mb-4 overflow-hidden border rounded-lg shadow-sm border-secondary-300 bg-secondary-200">
                <div class="flex flex-col justify-between sm:flex-row">
                    <div class="text-center sm:text-left flex-start">
                        <h3 class="text-lg font-semibold leading-6 text-gray-800">Hak Cipta</h3>
                        <p class="mt-px text-sm leading-5 text-gray-600 sm:mt-1">List Data Hak Cipta</p>
                    </div>
                    <div class="flex items-center justify-center mt-2 space-x-2 sm:mt-0">
                        <a href="{{ route('copyright.create') }}" class="inline-flex items-center justify-center px-4 py-2 text-sm font-bold tracking-wide text-white transition bg-blue-500 border border-transparent rounded-full shadow select-none focus:border-blue-600 hover:bg-blue-600 focus:outline-none focus:ring focus:ring-blue-500 focus:ring-opacity-30 disabled:opacity-50">
                            <svg class="w-5 h-5 -mx-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-11a1 1 0 10-2 0v2H7a1 1 0 100 2h2v2a1 1 0 102 0v-2h2a1 1 0 100-2h-2V7z" clip-rule="evenodd" />
                            </svg>
                        </a>
                        <a href="{{ route('copyright.export') }}" class="inline-flex items-center justify-center px-4 py-2 text-sm font-bold tracking-wide text-white transition bg-green-500 border border-transparent rounded-full shadow select-none focus:border-green-600 hover:bg-green-600 focus:outline-none focus:ring focus:ring-green-500 focus:ring-opacity-30 disabled:opacity-50">
                                    ...
                                    <div class="p-2">
                                        <form action="{{ url()->current() }}" method="get">
                                            <div class="relative mx-auto space-y-2">
                                                ...
                                                <h6 class="font-bold pl-2">Pemegang Hak Cipta</h6>
                                                <input type="search" name="pemegang_hak_cipta" value="{{ request('pemegang_hak_cipta') }}" class="block w-full pl-4 pr-10 text-sm leading-5 transition rounded-full shadow-sm border-secondary-300 bg-secondary-50 focus:bg-white focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                                                <h6 class="font-bold pl-2">Jenis Ciptaan</h6>
                                                <input type="search" name="creation_id" value="{{ request('creation_id') }}" class="block w-full pl-4 pr-10 text-sm leading-5 transition rounded-full shadow-sm border-secondary-300 bg-secondary-50 focus:bg-white focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                    ...
                    <table class="min-w-full">
                        ...
                        @if ($copyrights->count() > 0)
                            @foreach ($copyrights as $copyright)
                                <tr>
                                    <td class="px-6 py-3 leading-6 text-center whitespace-nowrap">{{ $copyrights->perPage() * ($copyrights->currentPage() - 1) + $loop->iteration }}</td>
                                    <td class="w-10 px-6 py-3 leading-6 sm:w-auto">{{ $copyright->judul_ciptaan }}</td>
                                    <td class="px-6 text-right select-none whitespace-nowrap">
                                        <a href="{{ route('copyright.show', $copyright) }}" class="inline-flex items-center justify-center px-4 py-2 text-sm font-medium tracking-wide text-white transition border border-transparent rounded-full shadow select-none bg-blue-500 focus:border-blue-600 hover:bg-blue-600 focus:outline-none focus:ring focus:ring-blue-500 focus:ring-opacity-30 disabled:opacity-50">
                                        ...
                                        <a href="{{ route('copyright.edit', $copyright) }}" class="inline-flex items-center justify-center px-4 py-2 text-sm font-medium tracking-wide text-white transition bg-yellow-500 border border-transparent rounded-full shadow select-none focus:border-yellow-600 hover:bg-yellow-600 focus:outline-none focus:ring focus:ring-yellow-500 focus:ring-opacity-30 disabled:opacity-50">
                                            <svg class="w-4 h-4 -mx-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                                <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                                            </svg>
                                        </a>
                                        <div endpoint="{{ route('copyright.destroy', $copyright) }}" class="inline delete"></div>
                                    </td>
                                </tr>
                            @endforeach
                        @else
