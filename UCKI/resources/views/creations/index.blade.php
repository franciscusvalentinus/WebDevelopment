            ...
            <div class="px-6 py-4 mb-4 overflow-hidden border rounded-lg shadow-sm border-secondary-300 bg-secondary-200">
                <div class="flex flex-col justify-between sm:flex-row">
                    <div class="text-center sm:text-left flex-start">
                        <h3 class="text-lg font-semibold leading-6 text-gray-800">Jenis Ciptaan</h3>
                        <p class="mt-px text-sm leading-5 text-gray-600 sm:mt-1">List Data Jenis Ciptaan</p>
                    </div>
                    <div class="flex items-center justify-center mt-2 space-x-2 sm:mt-0">
                        <a href="{{ route('creation.create') }}" class="inline-flex items-center justify-center px-4 py-2 text-sm font-bold tracking-wide text-white transition bg-blue-500 border border-transparent rounded-full shadow select-none focus:border-blue-600 hover:bg-blue-600 focus:outline-none focus:ring focus:ring-blue-500 focus:ring-opacity-30 disabled:opacity-50">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full">
                        ...
                        <tr>
                            ...
                            <th class="px-6 py-3 font-bold text-left uppercase">Creation Name</th>
                        ...
                        @if ($creations->count() > 0)
                            @foreach ($creations as $creation)
                                <tr>
                                    <td class="px-6 py-3 leading-6 text-center whitespace-nowrap">{{ $creations->perPage() * ($creations->currentPage() - 1) + $loop->iteration }}</td>
                                    <td class="w-10 px-6 py-3 leading-6 sm:w-auto">{{ $creation->creation }}</td>
                                    <td class="px-6 text-right select-none whitespace-nowrap">
                                        <a href="{{ route('creation.show', $creation) }}" class="inline-flex items-center justify-center px-4 py-2 text-sm font-medium tracking-wide text-white transition border border-transparent rounded-full shadow select-none bg-blue-500 focus:border-blue-600 hover:bg-blue-600 focus:outline-none focus:ring focus:ring-blue-500 focus:ring-opacity-30 disabled:opacity-50">
                                        ...
                                        <a href="{{ route('creation.edit', $creation) }}" class="inline-flex items-center justify-center px-4 py-2 text-sm font-medium tracking-wide text-white transition bg-yellow-500 border border-transparent rounded-full shadow select-none focus:border-yellow-600 hover:bg-yellow-600 focus:outline-none focus:ring focus:ring-yellow-500 focus:ring-opacity-30 disabled:opacity-50">
                                            <svg class="w-4 h-4 -mx-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                                <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                                            </svg>
                                        </a>
                                        <div endpoint="{{ route('creation.destroy', $creation) }}" class="inline delete"></div>
                                    </td>
                                </tr>
                            @endforeach
                        @else
