                    ...
                    <div class="shadow overflow-hidden sm:rounded-md">
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <input type="text" name="merek" id="merek" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('merek', $brand->merek) }}" autofocus/>
                            @error('merek')
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <input type="date" name="tanggal_permohonan" id="tanggal_permohonan" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('tanggal_permohonan', $brand->tanggal_permohonan) }}" />
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <input type="text" name="nomor_permohonan" id="nomor_permohonan" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('nomor_permohonan', $brand->nomor_permohonan) }}" />
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <input type="text" name="jenis_merek" id="jenis_merek" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('jenis_merek', $brand->jenis_merek) }}" />
                            @error('jenis_merek')
                            <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                                ...
                                @foreach($lecturers as $lecturer)
                                    @foreach ($departments as $department)
                                        @if ($lecturer->department_id == $department->id)
                                            <option value="{{ $lecturer->id }}" {{$brand->pencipta_1 == $lecturer->id  ? 'selected' : ''}}>{{ $lecturer->name}} ({{ $department->department }}) ({{ $lecturer->nidn }})</option>
                                        @endif
                                    @endforeach
                                @endforeach
                            </select>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <select type="text" name="pencipta_2" id="pencipta_2" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                ...
                                @foreach($lecturers as $lecturer)
                                    @foreach ($departments as $department)
                                        @if ($lecturer->department_id == $department->id)
                                            <option value="{{ $lecturer->id }}" {{$brand->pencipta_2 == $lecturer->id  ? 'selected' : ''}}>{{ $lecturer->name}} ({{ $department->department }}) ({{ $lecturer->nidn }})</option>
                                        @endif
                                    @endforeach
                                @endforeach
                            </select>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <select type="text" name="pencipta_3" id="pencipta_3" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                ...
                                @foreach($lecturers as $lecturer)
                                    @foreach ($departments as $department)
                                        @if ($lecturer->department_id == $department->id)
                                            <option value="{{ $lecturer->id }}" {{$brand->pencipta_3 == $lecturer->id  ? 'selected' : ''}}>{{ $lecturer->name}} ({{ $department->department }}) ({{ $lecturer->nidn }})</option>
                                        @endif
                                    @endforeach
                                @endforeach
                            </select>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <select type="text" name="pencipta_4" id="pencipta_4" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                ...
                                @foreach($lecturers as $lecturer)
                                    @foreach ($departments as $department)
                                        @if ($lecturer->department_id == $department->id)
                                            <option value="{{ $lecturer->id }}" {{$brand->pencipta_4 == $lecturer->id  ? 'selected' : ''}}>{{ $lecturer->name}} ({{ $department->department }}) ({{ $lecturer->nidn }})</option>
                                        @endif
                                    @endforeach
                                @endforeach
                            </select>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <select type="text" name="pencipta_5" id="pencipta_5" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                ...
                                @foreach($lecturers as $lecturer)
                                    @foreach ($departments as $department)
                                        @if ($lecturer->department_id == $department->id)
                                            <option value="{{ $lecturer->id }}" {{$brand->pencipta_5 == $lecturer->id  ? 'selected' : ''}}>{{ $lecturer->name}} ({{ $department->department }}) ({{ $lecturer->nidn }})</option>
                                        @endif
                                    @endforeach
                                @endforeach
                            </select>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <select type="text" name="pencipta_6" id="pencipta_6" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                ...
                                @foreach($lecturers as $lecturer)
                                    @foreach ($departments as $department)
                                        @if ($lecturer->department_id == $department->id)
                                            <option value="{{ $lecturer->id }}" {{$brand->pencipta_6 == $lecturer->id  ? 'selected' : ''}}>{{ $lecturer->name}} ({{ $department->department }}) ({{ $lecturer->nidn }})</option>
                                        @endif
                                    @endforeach
                                @endforeach
                            </select>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <select type="text" name="pencipta_7" id="pencipta_7" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                ...
                                @foreach($lecturers as $lecturer)
                                    @foreach ($departments as $department)
                                        @if ($lecturer->department_id == $department->id)
                                            <option value="{{ $lecturer->id }}" {{$brand->pencipta_7 == $lecturer->id  ? 'selected' : ''}}>{{ $lecturer->name}} ({{ $department->department }}) ({{ $lecturer->nidn }})</option>
                                        @endif
                                    @endforeach
                                @endforeach
                            </select>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <select type="text" name="pencipta_8" id="pencipta_8" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                ...
                                @foreach($lecturers as $lecturer)
                                    @foreach ($departments as $department)
                                        @if ($lecturer->department_id == $department->id)
                                            <option value="{{ $lecturer->id }}" {{$brand->pencipta_8 == $lecturer->id  ? 'selected' : ''}}>{{ $lecturer->name}} ({{ $department->department }}) ({{ $lecturer->nidn }})</option>
                                        @endif
                                    @endforeach
                                @endforeach
                            </select>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <select type="text" name="pencipta_9" id="pencipta_9" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                ...
                                @foreach($lecturers as $lecturer)
                                    @foreach ($departments as $department)
                                        @if ($lecturer->department_id == $department->id)
                                            <option value="{{ $lecturer->id }}" {{$brand->pencipta_9 == $lecturer->id  ? 'selected' : ''}}>{{ $lecturer->name}} ({{ $department->department }}) ({{ $lecturer->nidn }})</option>
                                        @endif
                                    @endforeach
                                @endforeach
                            </select>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <select type="text" name="pencipta_10" id="pencipta_10" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                ...
                                @foreach($lecturers as $lecturer)
                                    @foreach ($departments as $department)
                                        @if ($lecturer->department_id == $department->id)
                                            <option value="{{ $lecturer->id }}" {{$brand->pencipta_10 == $lecturer->id  ? 'selected' : ''}}>{{ $lecturer->name}} ({{ $department->department }}) ({{ $lecturer->nidn }})</option>
                                        @endif
                                    @endforeach
                                @endforeach
                            </select>
